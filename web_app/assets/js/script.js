...
const moviesUrl = 'https://api.themoviedb.org/3/search/movie?api_key=77e69574c201600a6f9114b3eb3478d0&query=';
...
function generateTrailerUrl(movieId) {
    var url = `https://api.themoviedb.org/3/movie/${movieId}/videos?api_key=77e69574c201600a6f9114b3eb3478d0&language=en-US`;
    return url;
}
...
function generateMovieCards(movies) {
    ...
    //aqui eu construo as estruturas em html. posso só fazer por fora direitinho depois transfiro pra cá
    /*const movieTemplate = `
    ...
    var section = generateMoviePoster(movies);
...
function generateMoviePoster(movies){
    ...
    movies.map((movie) => {  //.map itera sobre cada elemento do array invocando a função dada como parametro
        if(movie.poster_path){  //tentar entender depois o uso de `` em javascript e pq da formatação estranha da tag aí embaixo
