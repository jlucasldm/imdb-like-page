...
function generateMovieList(movies) {
  ...
  var list = generateMoviesInList(movies); //criando parte dos dados (até agora so tem trailer)
  //var content = document.createElement('div');
  //content.classList = 'content';
  //var contentClose = `<p id="content-close">dale dele doly nessa porra</p>`;
  //content.innerHTML = contentClose;

  movieList.appendChild(list); //movieList.appendChild(content);
...
function generateMoviesInList(movies) {
  ...
  for (var i = 0; i < movies.length; i++) {
    ...
    dataContainerSituacao.appendChild(dataSituacaoTitle); //criando estruturas fundamentais do card do filme
    ...
    if (movie.poster_path && movie.title && movie.vote_average && movie.release_date && movie.overview && movie.genre_ids) {
      ...
      detailsTitleCard.appendChild(detailsSpanTitle); //----------------inserindo porcentagem-----------------
      ...
      detailsSinopseText.appendChild(detailsSinopseP); //if (movie.genre_ids) {
      //var genreList = getGenre(genreUrl);
      //console.log('lista de generos ', genreList);
      //----------------inserindo genero-----------------
